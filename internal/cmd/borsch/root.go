...
import (
	...
	"github.com/YuriyLisovskiy/borsch-lang/internal/builtin/types"
	"github.com/YuriyLisovskiy/borsch-lang/internal/common"
	interpreter2 "github.com/YuriyLisovskiy/borsch-lang/internal/interpreter"
...
func runCode(code string) {
	run(
		func(i interpreter2.Interpreter) (types.Object, error) {
...
func run(fn func(i interpreter2.Interpreter) (types.Object, error)) {
	parser, err := interpreter2.NewParser()
	if err != nil {
		fmt.Println(err.Error())
		os.Exit(2)
	}
	...
	state := interpreter2.NewInitialState(nil, nil, stacktrace)
	i := interpreter2.NewInterpreter(parser, state)
	_, err = fn(i)
	if err != nil {
