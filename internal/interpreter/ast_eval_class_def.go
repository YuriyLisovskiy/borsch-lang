...
func (node *ClassDef) Evaluate(state State) (types2.Object, error) {
	// TODO: add doc
	// cls := &types.Class{
	...
	cls := types2.ObjectClass.ClassNew(node.Name.String(), map[string]types2.Object{}, node.IsFinal, nil, nil)
	...
	classContext.PushScope(map[string]types2.Object{})
	...
	if _, ok := cls.Dict[types2.InitializeMethodName]; ok {
	...
	if construct, ok := cls.Operators[common.ConstructorOp]; ok {
		cls.Dict[types2.InitializeMethodName] = construct
	}

	return cls, nil
}

func (node *ClassMember) Evaluate(state State, class *types2.Class) (types2.Object, error) {
	...
	if node.Method != nil {
		return node.Method.Evaluate(
			...
			true,
			func(arguments []types2.MethodParameter, returnTypes []types2.MethodReturnType) error {
	...
	if node.Operator != nil {
		operator, err := node.Operator.Evaluate(
			state,
			func(parameters []types2.MethodParameter, returnTypes []types2.MethodReturnType, opName string) error {
...
func checkMethod(class *types2.Class, args []types2.MethodParameter, _ []types2.MethodReturnType) error {
	if len(args) == 0 {
		// TODO: ukr error text!
...
func checkConstructor(_ []types2.MethodParameter, returnTypes []types2.MethodReturnType) error {
	switch len(returnTypes) {
	case 0:
		// skip
	case 1:
		if returnTypes[0].Class != types2.NilClass {
