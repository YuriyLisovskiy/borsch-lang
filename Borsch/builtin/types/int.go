...
var IntClass = ObjectClass.NewClass(
	"цілий", `
...
func IntNew(cls *Class, args Tuple) (Object, error) {
	var xObj Object = Int(0)
	var baseObj Object
	base := 0
	err := ParseExactArgs(args, "цілий:OO", &xObj, &baseObj)
	if err != nil {
		return nil, err
	}
...
func cantConvert(a Object, to string) (Object, error) {
	return nil, ErrorNewf(TypeError, "неможливо перетворити %s у %s", a.Class().Name, to)
}
