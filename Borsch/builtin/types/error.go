...
func ErrorNewf(cls *Type, format string, a ...interface{}) error {
...
func ErrorNew(cls *Type, args Tuple, kwargs StringDict) (Object, error) {
	if len(kwargs) != 0 {
		return nil, fmt.Errorf("ПомилкаТипу: %s не приймає аргументи у вигляді словника", cls.Name)
	}

	return errorNew(cls, args), nil
}

func errorNew(cls *Type, args Tuple) *Error {
	return &Error{
		Base: cls,
		Args: args.Copy(),
		Dict: StringDict{},
