...
var ListClass = ObjectClass.NewClass(
	"список",
...
func init() {
	// ListClass.Dict["append"] = MustNewMethod(
	...
	// 	}, 0, "append(item)",
	// )
	// ListClass.Dict["sort"] = MustNewMethod(
	// 	"sort", func(self Object, args Tuple, kwargs Dict) (Object, error) {
	// 		const funcName = "sort"
	// 		l, isList := self.(*List)
	// 		if !isList {
...
func ListNew(cls *Class, args Tuple) (res Object, err error) {
	var iterable Object
	err = UnpackTuple(args, "список", 0, 1, &iterable)
	if err != nil {
		return nil, err
	}
...
func (s ptrSortable) Less(i, j int) bool {
	...
	if s.s.keyFunc != Nil {
		itemI, err = Call(nil, s.s.keyFunc, Tuple{itemI})
		if err != nil {
			if s.s.firstErr == nil {
				s.s.firstErr = err
			}
		...
		itemJ, err = Call(nil, s.s.keyFunc, Tuple{itemJ})
		if err != nil {
			if s.s.firstErr == nil {
				s.s.firstErr = err
			}
...
func SortInPlace(l *List, funcName string) error {
	var keyFunc Object
	var reverse Object
	err := ParseExactArgs(nil, funcName+":OB", &keyFunc, &reverse)
	if err != nil {
		return err
	}
