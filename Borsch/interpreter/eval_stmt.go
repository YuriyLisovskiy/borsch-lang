...
func (node *Stmt) Evaluate(state types.State, inFunction, inLoop bool) StmtResult {
	switch {
	case node.Throw != nil:
	...
	case node.FunctionDef != nil:
		// function, err := node.FunctionDef.Evaluate(state, state.GetCurrentPackage().(*types.Package))
		function, err := node.FunctionDef.Evaluate(state, nil)
		if err != nil {
