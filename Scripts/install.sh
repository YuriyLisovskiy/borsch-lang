...
echo
echo -e "${C_BOLD_PURPLE}==>${NO_COLOR} ${C_BOLD_BLACK}Завершення процесу встановлення.${NO_COLOR}"
echo
...
echo -e "${C_BOLD_BLACK}  alias борщ="${BORSCH_BIN}/${APP_NAME}"${NO_COLOR}"
echo
echo Перезапустіть термінал, щоб застосувати зміни. Для цього оновіть профіль. Приклад команди для оболонки Bash:
echo -e "${C_BOLD_BLACK}  source ~/.bash_profile${NO_COLOR}"
echo
echo Для застосування змін в інших оболонках, дізнайтеся самостійно, як це зробити, або просто перезапустіть термінал.
echo
